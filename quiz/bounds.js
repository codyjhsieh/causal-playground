export const questions = [
  ...
  },
  {
    ...
    answer: [0, 1, 3],
    explain: "A is correct: MTS constrains unobserved Y(1) for controls to be at most E[Y|T=1] and unobserved Y(0) for the treated to be at least E[Y|T=0], yielding UB_MTS = E[Y|T=1] − E[Y|T=0] — the observed arm difference. B is correct and is the key NSW insight: because NSW is randomized there is no selection bias, so the observed arm difference equals the true ATE — MTS collapses the upper bound directly to the causal truth. D is correct: MTS formalizes the intuition that self-selection into training is positively correlated with potential outcomes, bounding unobserved counterfactuals by the observed arm means. C is false: MTS tightens the <em>upper</em> bound; the <em>lower</em> bound is tightened by MTR (Monotone Treatment Response). In this module the code sets LB_MTS = 0 because the MTS-only lower bound algebraically equals zero for binary outcomes — but this zero lower bound is a weaker constraint than MTR's, which also gives LB ≥ 0 via a distinct assumption."
  },
  {
