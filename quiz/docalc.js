export const questions = [
  ...
  },
  {
    ...
    answer: 1,
    explain: "B is correct. The backdoor graph is identifiable via Σ_z P(Y|X,z)P(z). The front-door graph is identifiable via the front-door formula (Pearl 1995). The <strong>bow arc</strong> (X→Y with a bidirected/latent arc U between X and Y) is the canonical non-identifiable graph — no amount of data or do-calculus rules can eliminate the do-operator; this is proven by the hedge criterion (Shpitser &amp; Pearl 2006). The instrument graph: IV identifies the LATE (local ATE for compliers) under monotonicity, but not the population ATE without additional homogeneity assumptions — so it is not point-identified in the do-calculus sense. A and C are wrong. D is wrong: you cannot condition on U because it is unobserved; IV ratio gives LATE, not ATE."
  },
  {
  ...
  },
  {
    ...
    answer: 1,
    explain: "Non-identifiability is an information-theoretic statement about the graph structure, proven by Shpitser &amp; Pearl's (2006) hedge criterion. Formally: two distributions can agree on all observational probabilities P(V) while disagreeing on P(Y | do(X)). More data let you estimate P(V) more precisely, but they cannot distinguish two models that generate the same P(V). Asymptotic arguments (A, C) misunderstand the nature of the result — it holds in the infinite-data limit too. D is wrong because randomization would itself be an intervention (do(X)), not an observational dataset; the point is about observational data."
  },
  {
